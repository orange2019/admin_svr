  ...
  <div class="page-news-update">
    ...
    <form @submit.prevent="newsUpdateSubmit()" class="form">
      ...
      <div class="form-group row">
        ...
        <div class="col-9">
          <textarea class="form-control" rows="10" v-model="newsData.content"  placeholder="请输入文章内容" id="news-content" v-html="newsData.content"></textarea>
        </div>
      ...
      <div class="form-group row">
        ...
        <div class="col-7">
          <input type="text" v-model.lazy="newsData.cover" required class="form-control" id="news-cover-input">
          <div>
            <img :src="newsData.cover" alt="" id="news-cover-pre" height="100" class="mt-1">
          </div>
        </div>
        <div class="col-2">
          <input type="button" value="上传图片" id="news-cover-btn" class="btn btn-primary btn-block btn-sm">
        </div>
      </div>
...
let initKEditor = (Editor , store , cb) => {
  ...
  if (document.getElementById("news-cover-btn")) {
    window.imgBtn = Editor.createImgBtn(
      "news-cover-btn",
      store
    );
  }
